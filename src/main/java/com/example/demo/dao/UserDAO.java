...
public interface UserDAO extends DAO<User> {
    @Override
    @SqlUpdate("INSERT INTO user (username, password, fullname, gender, phoneNumber, email, cccd, role, imageName,bankAccount,nameBankAccount,valid)" +
            "VALUES (:username, :password, :fullname, :gender, :phoneNumber, :email, :cccd, :role, :imageName,:bankAccount,:nameBankAccount,:valid)")
    @GetGeneratedKeys("id")
    ...
    @SqlQuery(" SELECT u.* " +
            ...
            "WHERE p.id = :id ")
    Optional<User> getByUserId(@Bind("id") long id);
    @SqlQuery("SELECT * FROM user WHERE id = :id ")
    Optional<User> getUserIdVer2(@Bind("id") long id);

    @SqlQuery(" SELECT * FROM user where role = :role && valid = :valid ")
    List<User> getAllByConditionsAndValid(@Bind("role") String role ,@Bind("valid") int valid);
    @SqlQuery("SELECT * FROM user " +
            "WHERE role = :role AND valid = :valid " +
            "ORDER BY :orderBy :orderDir LIMIT :limit OFFSET :offset")
    List<User> getFilteredAndOrderedUsers(@Bind("role") String role,
                                          @Bind("valid") int valid,
                                          @Bind("orderBy") String orderBy,
                                          @Bind("orderDir") String orderDir,
    ...
    @SqlUpdate("UPDATE user SET valid = 404 WHERE id = :id")
    void hide(@Bind("id") long id);

    @SqlUpdate("UPDATE user SET valid = 202  WHERE id = :id")
